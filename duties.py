...
@duty(pre=["check-quality", "check-types", "check-deps", "check-docs", "check-api"])
...
@duty(nofail=PY_VERSION == PY_DEV)
def check_deps(ctx: Context) -> None:
    """Check for dependency issues."""
    ctx.run(
        ["deptry", "src"],
        title=pyprefix("Checking dependencies"),
    )
...
def check_api(ctx: Context, *cli_args: str) -> None:
    ...
    ctx.run(
        tools.griffe.check("voxpipe", search=["src"], color=True).add_args(*cli_args),
